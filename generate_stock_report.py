...
class Stock():
    ...
    need_items = [
        '营业总收入(万元)', '研发费用(万元)', '财务费用(万元)', '净利润(万元)_y', '归属于母公司所有者的净利润(万元)',
        '总资产(万元)', '总负债(万元)', '流动资产(万元)', '流动负债(万元)', '股东权益不含少数股东权益(万元)',
        '净资产收益率加权(%)', ' 支付给职工以及为职工支付的现金(万元)', '经营活动产生的现金流量净额(万元)',
        ' 投资活动产生的现金流量净额(万元)', '应收账款(万元)', '存货(万元)', '开发支出(万元)',
        '归属于母公司股东权益合计(万元)', '所有者权益(或股东权益)合计(万元)', '投资收益(万元)_x',
        '实收资本(或股本)(万元)', '每股净资产(元)'
    ]
    # ,'支付给职工以及为职工支付的现金(万元)'

    items_new_name = [
        '营业收入', '研发费用', '财务费用', '净利润', '归属净利润', '总资产', '总负债', '流动资产', '流动负债',
        '股东权益', 'ROE', ' 职工薪酬', '经营现金流', '投资现金流', '应收账款', '存货', '开发支出',
        '归属股东权益', '权益合计', '投资收益', '总股本', '每股净资产'
    ]  # 名字和上面列表一一对应
    ...
    def __init__(self, code, name):
        ...
        self.name = name
        # self.dframe=dframe
        self.cwzb_path = os.path.join(self.caiwu_folder,
                                      '财务指标' + self.code + '.csv')
        self.zcfzb_path = os.path.join(self.caiwu_folder,
                                       '资产负债表' + self.code + '.csv')
        self.lrb_path = os.path.join(self.caiwu_folder,
                                     '利润表' + self.code + '.csv')
        self.xjllb_path = os.path.join(self.caiwu_folder,
                                       '现金流量表' + self.code + '.csv')
        ...
        self.url_reports = {
            self.cwzb_url: self.cwzb_path,
            self.zcfzb_url: self.zcfzb_path,
            self.lrb_url: self.lrb_path,
            self.xjll_url: self.xjllb_path
        }
    ...
    def _generate_report(self):
        ...
        for key in self.url_reports:
            data_frames.append(
                pd.read_csv(
                    self.url_reports[key], encoding="gbk", index_col=0).T)
        merge_frame = None
        for frame in data_frames:
            if merge_frame is None:
                ...
                merge_frame = pd.merge(
                    merge_frame, frame, left_index=True, right_index=True)
        ...
        for index, row in merge_frame.iterrows():
            try:
                ...
                pass
        try:
            merge_frame['自由现金流'] = pd.to_numeric(
                merge_frame['经营现金流']) + pd.to_numeric(merge_frame['投资现金流'])
        except Exception as e:
            merge_frame['自由现金流'] = 0
            print(self.name + '自由现金流计算失败')

        try:
            merge_frame['负债率'] = pd.to_numeric(
                merge_frame['总负债']) / pd.to_numeric(merge_frame['总资产'])
            merge_frame['负债率'] = merge_frame['负债率'].round(2)
        except Exception as e:
            merge_frame['负债率'] = 0.66
            print(self.name + '负债率计算失败')
        try:
            merge_frame['流动比率'] = pd.to_numeric(
                merge_frame['流动资产']) / pd.to_numeric(merge_frame['流动负债'])
            merge_frame['流动比率'] = merge_frame['流动比率'].round(1)
        except Exception as e:
            merge_frame['流动比率'] = 0.66
            print(self.name + '流动比率计算失败')
        merge_frame = merge_frame[[
            '营业收入', '流动比率', '负债率', '经营现金流', '投资现金流', '自由现金流', '净利润', '投资收益',
            ' 职工薪酬', '财务费用', '研发费用', '开发支出', 'ROE', '总股本', '每股净资产'
        ]]
    ...
    def get_3year_average_fh(self):  # 单位为万元
        try:
            ...
            sum_fh = float(fd[1].contents[1].string.replace(',', '')) + float(
                fd[2].contents[1].string.replace(',', '')) + float(
                    fd[3].contents[1].string.replace(',', ''))
            # print(sum_fh)
...
def update_fhlv():
    ...
    stocks['平均分红率'] = stocks['3年平均分红'] * 100 / (
        stocks['总股本(万)'] * stocks['价格'])
